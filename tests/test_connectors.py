...
# LangSmith connector tests (with mocking)


@patch("os.getenv")
def test_langsmith_connector_missing_api_key(mock_getenv):
    """LangSmith connector requires API key."""
...
@patch("driftbase.connectors.langsmith.LangSmithClient")
@patch("os.getenv")
def test_langsmith_validate_credentials_success(mock_getenv, mock_client_class):
    """LangSmith credential validation succeeds with valid key."""
    mock_getenv.return_value = "test-api-key"
    mock_client = MagicMock()
    mock_client.list_projects.return_value = iter([{"name": "test"}])
...
@patch("driftbase.connectors.langsmith.LangSmithClient")
@patch("os.getenv")
def test_langsmith_validate_credentials_failure(mock_getenv, mock_client_class):
    """LangSmith credential validation fails with invalid key."""
    mock_getenv.return_value = "invalid-key"
    mock_client = MagicMock()
    mock_client.list_projects.side_effect = Exception("Invalid key")
    mock_client_class.return_value = mock_client
...
@patch("driftbase.connectors.langsmith.LangSmithClient")
@patch("os.getenv")
def test_langsmith_map_trace_basic(mock_getenv, mock_client_class):
    """LangSmith trace mapping produces valid Driftbase run."""
    mock_getenv.return_value = "test-api-key"
    ...
    trace = {
        ...
        "error": None,
        "child_runs": [
            {"run_type": "tool", "name": "check_policy"},
            {"run_type": "tool", "name": "send_email"},
        ],
    }
    ...
    assert run["external_id"] == "trace-123"
    assert run["source"] == "langsmith"
    assert run["deployment_version"] == "v1.0"
    assert run["latency_ms"] == 5000
    assert run["prompt_tokens"] == 100
    assert run["completion_tokens"] == 200
    assert run["error_count"] == 0
    assert run["semantic_cluster"] == "resolved"
