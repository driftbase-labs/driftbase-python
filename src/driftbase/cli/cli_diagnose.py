...
def _calculate_metrics(runs: list[dict[str, Any]]) -> dict[str, float]:
    ...
    return {
        ...
        "p95_latency": sorted([r.get("latency_ms", 0) for r in runs])[int(total * 0.95)] if total > 1 else 0,
        "avg_prompt_tokens": sum(r.get("prompt_tokens", 0) for r in runs) / total,
        "avg_completion_tokens": sum(r.get("completion_tokens", 0) for r in runs) / total,
        ...
        "escalation_rate": sum(1 for r in runs if r.get("semantic_cluster") == "escalated") / total,
        "avg_tool_count": sum(r.get("tool_call_count", 0) for r in runs) / total,
    }


def _detect_regression_type(baseline_metrics: dict[str, float], current_metrics: dict[str, float]) -> list[tuple[str, float]]:
    ...
    prompt_increase = (current_metrics.get("avg_prompt_tokens", 0) - baseline_metrics.get("avg_prompt_tokens", 1)) / baseline_metrics.get("avg_prompt_tokens", 1)
    completion_increase = (current_metrics.get("avg_completion_tokens", 0) - baseline_metrics.get("avg_completion_tokens", 1)) / baseline_metrics.get("avg_completion_tokens", 1)
    ...
    # Loop detection
    loop_increase = (current_metrics.get("avg_loop_count", 0) - baseline_metrics.get("avg_loop_count", 1)) / baseline_metrics.get("avg_loop_count", 1)
    retry_increase = (current_metrics.get("avg_retry_count", 0) - baseline_metrics.get("avg_retry_count", 0.5)) / baseline_metrics.get("avg_retry_count", 0.5)
    ...
    tool_decrease = (baseline_metrics.get("avg_tool_count", 1) - current_metrics.get("avg_tool_count", 0)) / baseline_metrics.get("avg_tool_count", 1)
    ...
    # Latency creep
    latency_increase = (current_metrics.get("avg_latency", 0) - baseline_metrics.get("avg_latency", 1)) / baseline_metrics.get("avg_latency", 1)
...
def _compare_to_benchmark(metrics: dict[str, float], benchmark_name: str) -> dict[str, str]:
...
def _generate_recommendations(
    ...
    if current_metrics.get("escalation_rate", 0) > baseline_metrics.get("escalation_rate", 0) * 2:
        recommendations.append("⚠️ Escalation rate doubled - agent confidence may have decreased")
...
def cmd_diagnose(
    ...
    metrics_table.add_row("Prompt Tokens (avg)", f"{current_metrics['avg_prompt_tokens']:.0f}")
    metrics_table.add_row("Completion Tokens (avg)", f"{current_metrics['avg_completion_tokens']:.0f}")
    metrics_table.add_row("Loop Count (avg)", f"{current_metrics['avg_loop_count']:.1f}")
    metrics_table.add_row("Retry Count (avg)", f"{current_metrics['avg_retry_count']:.1f}")
    metrics_table.add_row("Error Rate", f"{current_metrics['error_rate']:.1%}")
    metrics_table.add_row("Escalation Rate", f"{current_metrics['escalation_rate']:.1%}")
    metrics_table.add_row("Tool Calls (avg)", f"{current_metrics['avg_tool_count']:.1f}")
    ...
    if compare:
        ...
        if not baseline_runs:
            ...
            for key in ["avg_latency", "avg_prompt_tokens", "avg_completion_tokens", "avg_loop_count", "avg_retry_count", "error_rate", "escalation_rate"]:
            ...
            regression_matches = _detect_regression_type(baseline_metrics, current_metrics)
            ...
            recommendations = _generate_recommendations(regression_matches, baseline_metrics, current_metrics)
